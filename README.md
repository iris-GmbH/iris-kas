...
| KAS_TARGET_RECIPE | Defines one or more bitbake recipes to build. Common image targets include `irma6-maintenance`, `irma6-deploy`, `irma6-dev`, `irma6-base` | `irma6-maintenance` |
...
We list the most common use-cases here, however covering all possible options here would go beyond the scope of this README. For this, please take a look at the content of the Makefile.

#### Building the irma6-base image

iris-GmbH is committed to open source license compliance. Thus, anyone can fetch the sources for `irma6-base`, which is our custom embedded Linux distribution (minus our proprietary code) and built it for themselves.

1. Build the iris-kas build container image: `make build-iris-kas-container`
2. Fetch sources and build the `irma6-base` image: `IRIS_KAS_CONTAINER_PULL=never KAS_TARGET_RECIPE=irma6-base make`

#### Running arbitrary commands in the KAS shell (make kas(-interactive)-shell)

If you find the [kas build plugin](https://kas.readthedocs.io/en/latest/userguide.html#module-kas.plugins.build) to be too restricting, you may opt to use the [kas shell plugin](https://kas.readthedocs.io/en/latest/userguide.html#module-kas.plugins.shell) instead by running `make kas-shell`.

By default, `kas-shell` behaves identical to `kas-build`, however by overriding the `KAS_SHELL_CMD` variable, you may run any arbitrary shell command within the KAS build environment, e.g.:

`KAS_SHELL_CMD="echo 'hello world'" make kas-shell`

Alternatively, you may run `make kas-interactive-shell` for opening an interactive shell.

#### Running KAS manually

In very rare situations, it might become necessary to call `kas` directly. This should be avoided and only used as a last resort, as common configurations done by the Makefile will be missing (e.g. DISTRO_VERSION will be 0.0.0-unknown).

In the case of a local KAS installation, this can be done by calling the `kas` binary directly, e.g.:

`kas shell -c "bitbake foo" kas-irma6-base.yml:kas-irma6-pa.yml`.
