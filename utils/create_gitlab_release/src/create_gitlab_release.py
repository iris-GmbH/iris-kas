...
def get_artifact_path(conf: str, project_dir: str, artifact_type: str, logging: logging.Logger) -> str:
  """
  Retrieves the artifact path for a given conf and artifact type from the approriate environment variable (previously set by CI).
  """
...
def upload_artifacts_to_registry(gl: Gitlab, multi_conf: str, artifacts: dict, gitlab_project: Project, project_name: str, release_version: str, dry_run: bool, logging: logging.Logger):
  """
  ...
  for artifact_type in artifacts:
    ...
    if dry_run is False:
      logging.debug(f'Uploading artifact_type {artifact_type} for multi_conf {multi_conf} at path {path} to registry')
...
def generate_release_description(multi_conf: str, artifacts: dict, logging: logging.Logger) -> str:
  ...
  for artifact_type in artifacts:
    artifact_name = os.path.basename(artifacts[artifact_type][multi_conf]['path'])
...
def run(gitlab_server_url: str, gitlab_access_token: str, dry_run: bool, multi_conf: str, kas_artifact_prefix: str, project_dir: str, gitlab_project_id: int, release_name: str, release_version: str, logging: logging.Logger):
  ...
  with tempfile.TemporaryDirectory() as tmpdir:
    ...
    upload_artifacts_to_registry(gl, multi_conf, artifacts, gitlab_project, release_name, release_version, dry_run, logging)
