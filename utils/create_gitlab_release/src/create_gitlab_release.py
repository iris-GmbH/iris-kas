...
def prepare_release_artifacts(release_multi_confs: list, project_dir: str, tmpdir: tempfile.TemporaryDirectory, kas_artifact_prefix: str, logging: logging.Logger) -> dict:
  # All release confs must contain the artifacts "deploy", "dev_deploy", "maintenance", "dev", "sdk", "deploy_customer" and "kas_environment":
  artifacts = {
    'deploy': {
    ...
    'dev_deploy': {
      ...
      'clearance': 'Internal/External (ONLY for license compliance source code)'
    },
    'maintenance': {
  ...
  for conf in release_multi_confs:
    create_deploy_customer_artifact_files(conf, project_dir, tmpdir, kas_artifact_prefix, logging)
    for artifact_type, _ in artifacts.items():
      artifact_path = get_artifact_path(conf, project_dir, artifact_type, logging)
...
def get_conf_var_name(conf: str) -> str:
  # translate multi_conf into valid variable name
  return conf.replace('-', '_')


def get_artifact_path(conf: str, project_dir: str, artifact_type: str, logging: logging.Logger) -> str:
...
def create_deploy_customer_artifact_files(conf: str, project_dir: str, tmpdir: tempfile.TemporaryDirectory, kas_artifact_prefix: str, logging: logging.Logger) -> None:
  logging.info(f'Creating deploy customer artifact files for conf {conf}')
  deploydir = get_artifact_path(conf, project_dir, "deploy", logging)
  logging.debug(f'deploy directory for conf {conf} has been identified as {deploydir}')
  logging.info(f'Identifying customer deploy artifacts for conf {conf}')
  artifact_paths = []
  ...
  if conf == "sc573-gen6":
    ...
    if len(artifact_paths) != 2:
      raise ValueError(f"Could not clearly identify firmware and bootloader update files for {conf}")
  ...
  # newer releases use swupdate
    ...
    if len(artifact_paths) != 1:
      raise ValueError(f"Could not clearly identify swupdate file for {conf}")

  dirname = os.path.join(tmpdir, f'{kas_artifact_prefix}{conf}-deploy-customer')
...
def create_gitlab_release(release_multi_confs: list, artifacts: dict, gl: Gitlab, gitlab_project: Project, release_version: str, dry_run: bool, logging: logging.Logger):
  release_description = generate_release_description(release_multi_confs, artifacts, logging)
...
@click.option('--project-dir', envvar='CI_PROJECT_DIR', type=str, help='The absolute path to the folder where the raw build artifacts have been stored beforehand')
@click.option('--release-version', envvar='CI_COMMIT_TAG', type=str, required=True, help='The release version')
def cli(gitlab_server_url: str, gitlab_access_token: str, dry_run: bool, release_multi_confs: list, kas_artifact_prefix: str, project_dir: str, gitlab_project_id: int, release_version: str):
