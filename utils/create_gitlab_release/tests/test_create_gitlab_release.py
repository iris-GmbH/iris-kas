...
def test_without_artifacts_files(authed_gitlab: Gitlab, group: Group, tmpdir):
...
def test_create_gitlab_release_r1_only(authed_gitlab: Gitlab, group: Group, tmpdir):
  logging.basicConfig(level=logging.DEBUG)
  project_dir = tempfile.mkdtemp()
  confs = ['sc573-gen6']
  project_dir = tmpdir.mkdir("project_dir")
...
def test_create_gitlab_release_r2_only(authed_gitlab: Gitlab, group: Group, tmpdir):
  logging.basicConfig(level=logging.DEBUG)
  project_dir = tempfile.mkdtemp()
  confs = ['imx8mp-irma6r2']
  project_dir = tmpdir.mkdir("project_dir")
  create_test_artifact_vars(project_dir, confs[0], logging)
  create_test_artifact_files(project_dir, confs[0], logging)
  project = create_project_in_group(authed_gitlab, group, 'test-project')
...
def test_create_gitlab_release_all(authed_gitlab: Gitlab, group: Group, tmpdir):
  logging.basicConfig(level=logging.DEBUG)
  project_dir = tempfile.mkdtemp()
